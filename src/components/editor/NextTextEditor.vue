...
import { configureSchemas, FILE_PREFIX } from './functions';
...
watch(() => props.modelFileUri, (newFileUri) => {
    log.info('Changing modelFileUri to ' + newFileUri)
...
watch(() => props.annotations, async (inputs) => {
    ...
    log.info(`Adding ${decorations.length} to editor decorations`);
...
onMounted(async () => {
    ...
    _editor.getModel()?.onDidChangeContent(e => {
